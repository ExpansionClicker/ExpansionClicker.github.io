...
function preloadImages() {
  for (i = 0; i < imagePaths.length; i++) {
    ...
    document.getElementById("preloadLoadingBar").style.width = (100*(i/(vidPaths.length + imagePaths.length))+1) + "%";
    document.getElementById("currentPreloadingImage").innerHTML = imagePaths[i];
	}
  console.log("Images Preloaded: " + preloadedImages.length + " / " + imagePaths.length)
}

//Video Preloader
var preloadedVids = new Array()
function preloadVids() {
  for (i = 0; i < vidPaths.length; i++) {
    preloadedVids[i] = document.createElement('video');
    preloadedVids[i].src = vidPaths[i]
    document.getElementById("preloadLoadingBar").style.width = (100*((i+preloadedImages.length)/(vidPaths.length + imagePaths.length))+1) + "%";
    document.getElementById("currentPreloadingImage").innerHTML = vidPaths[i];
  }
  console.log("Videos Preloaded: " + preloadedVids.length + " / " + vidPaths.length)
}
